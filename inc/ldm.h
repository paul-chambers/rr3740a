...
struct vdev_class *ldm_find_vdev_class(PVBUS vbus, HPT_U8 type);
...
void * ldm_get_vbus_osext(PVBUS vbus);
...
int ldm_initialize_vbus_async(PVBUS vbus, PLDM_ADAPTER master_adapter, void (*done)(void *osext));
...
void ldm_set_vbus_invalid(void * pVbus, HPT_BOOL invalid);

void ldm_release_vbus(PVBUS vbus);
void ldm_release_vbus2(PVBUS vbus);
...
void ldm_pause(PVBUS vbus,HPT_BOOL IsQuiesce);
...
void * vbus_to_him_handle(PVBUS vbus);
HIM * vbus_to_him(PVBUS vbus);
