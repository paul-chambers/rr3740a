			       ...
			       u8 *asc, u8 *ascq)
{
 ...
 translate_done:
		KdPrint(("translated ATA stat/err 0x%02x/%02x "
		       "to SCSI SK/ASC/ASCQ 0x%x/%02x/%02x\n",
		       drv_stat, drv_err, *sk, *asc, *ascq));
	return;
}
	...
	case INQUIRY:
	{
		...
		if (SCpnt->cmnd[1] & 1) {
			...
			case 0x83:
				...
				if (mIsArray(pVDev->type)){
					rbuf[2] = 0;
					rbuf[3] = 12; /* page length (n-3) */
					rbuf[4] = 1; /*code set Binary*/
					rbuf[5] = 2; /* identifier type - EUI-64 */
					rbuf[6] = 0;
					rbuf[7] = 8; /* identifier length */
					rbuf[8] = 0; /* HPT IEEE ID - 0x00193C */
					rbuf[9] = 0x19;
					rbuf[10] = 0x3C;
					memcpy(rbuf+11, &pVDev->u.array.array_sn, 4);
					rbuf[15] = 0;
					SCpnt->result = (DID_OK<<16);
				}else{
					HPT_U8 vender_tmp[8];
					...
					pVDev->u.raw.him->get_device_config(pVDev->u.raw.phy_dev,&devconf);
					rbuf[2] = 0;
					rbuf[3] = 72; /* page length (n-3) */
					rbuf[4] = 2;	 /*code set ASCII*/
					rbuf[5] = 1; /* identifier type - T10 vendor ID based */
					rbuf[6] = 0;
					rbuf[7] = 68; /* identifier length */	

					for (i=0; i<8 ; i+=2) {
						vender_tmp[i] = ((HPT_U8*)(devconf.pIdentifyData->ModelNumber))[i+1];
						vender_tmp[i+1] = ((HPT_U8*)(devconf.pIdentifyData->ModelNumber))[i];
					}
					...
					if (i == 8){
						if (((vender_tmp[0] == 'I') && (vender_tmp[1] == 'C')) ||
								((vender_tmp[0] == 'H') && (vender_tmp[1] == 'T')) ||
								((vender_tmp[0] == 'H') && (vender_tmp[1] == 'D')) ||
								((vender_tmp[0] == 'D') && (vender_tmp[1] == 'K'))){
							memcpy(rbuf+8, "Hitachi ", 8);
						}
						else if ((vender_tmp[0] == 'S') && (vender_tmp[1] == 'T')){
							memcpy(rbuf+8, "Seagate ", 8);
						}else{
							/*Unkown*/
							memcpy(rbuf+8, "SATA    ", 8);
						}
					}else{
						int j = i;
						memcpy(rbuf+8, vender_tmp, j);
						for (; j < 8; j++)
							rbuf[8+j] = ' ';
					}

					memcpy(rbuf+16, devconf.pIdentifyData->ModelNumber, 40);
					ldm_ide_fixstring(rbuf+16, 40);
					memcpy(rbuf+56, devconf.pIdentifyData->SerialNumber, 20);
					ldm_ide_fixstring(rbuf+56, 20);
...
static int hpt_slave_config(struct scsi_device *sdev)
{
	...
	if (sdev->type == TYPE_TAPE) {
		...
		sdev->request_queue->rq_timeout = 150 * HZ;
